...
/* Actual period: 1 seconds. Actual duty: 50%. */.period_counts = (uint32_t) 0x2dc6c00,
  .duty_cycle_counts = 0x16e3600, .source_div = (timer_source_div_t) 0, .channel = 0, .p_callback = timer_callback,
...
adc_instance_ctrl_t g_adc_pot_ctrl;
const adc_extended_cfg_t g_adc_pot_cfg_extend =
...
#if ((0) | (0))
const adc_window_cfg_t g_adc_pot_window_cfg =
{
    .compare_mask        =  0,
    .compare_mode_mask   =  0,
...
#endif
const adc_channel_cfg_t g_adc_pot_channel_cfg =
...
#if ((0) | (0))
    .p_window_cfg        = (adc_window_cfg_t *) &g_adc_pot_window_cfg,
#else
  .p_window_cfg = NULL,
#endif
        };
/* Instance structure to use this module. */
const adc_instance_t g_adc_pot =
{ .p_ctrl = &g_adc_pot_ctrl, .p_cfg = &g_adc_pot_cfg, .p_channel_cfg = &g_adc_pot_channel_cfg, .p_api = &g_adc_on_adc };
...
{ .channel = 1, .rate = I2C_MASTER_RATE_STANDARD, .slave = 0x3C, .addr_mode = I2C_MASTER_ADDR_MODE_7BIT,
#define RA_NOT_DEFINED (1)
#if (RA_NOT_DEFINED == RA_NOT_DEFINED)
  .p_transfer_tx = NULL,
#else
